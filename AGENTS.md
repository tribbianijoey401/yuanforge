...
3. grep 自己角色的任务行 + 上下文传递行（~8行）                     ✅ 必读
4. 读铁律 .yuan/rules/iron-rules.md（~2KB）                        ✅ 必读
5. 读自己的角色合约 contracts/<角色>.md                             ✅ 必读
6. 读 knowledge/pitfalls/（避坑）                                   ✅ 必读
7. 读上游产出物文件（按需）
```
...
| 文件 | 内容 | 读？ |
|------|------|:---:|
| .yuan/rules/iron-rules.md | 九条铁律 — 含三档阻塞策略 | ✅ 每次必读 |
