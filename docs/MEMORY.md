...
### M-019：状态规范必须有单一可执行提交门

- **Symptom**：LLM 把自由动作或 Persona 名称写入规范 `stage` / `agent.id`；Insight 虽能事后报警，但 Workflow 已由非法状态继续执行。
- **Cause**：规范仅存在于分散自然语言，Installer 与 Insight 又各自维护校验逻辑，没有 State Commit 级硬门。
- **Rule**：Canonical Source 统一到 `state-contract.md`；Workflow Stage 来自当前 Workflow frontmatter，Agent ID 来自 Agent Contract 文件名且必须被当前 Workflow 声明。每次 Conductor Commit 后运行同一只读 State Guard，未得到 `STATE_VALID` 不得 Dispatch。具体动作只维护在 WORK Current Task，Insight 从中派生展示；执行实例标签可放入 `agent.instance`。Installer 与 Insight 只复用 Guard 结果。

### M-020：`hidden` 必须验证计算样式而不只验证 DOM 属性

- **Symptom**：组件已有 `hidden=true`，但 author CSS 的 `display:flex/grid` 覆盖 UA 默认 `[hidden] { display:none }`，导致健康状态仍显示错误条带。
- **Rule**：使用 `hidden` 控制自定义布局组件时显式声明 `<component>[hidden] { display:none; }`；回归同时检查 DOM 属性与浏览器计算后的实际可见性，并验证失败态能恢复显示。
