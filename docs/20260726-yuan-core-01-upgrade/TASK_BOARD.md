...
| **活跃 Agent** | 无；task-003 r1 已修复，下一任务 task-004 复测 |
...
| task-003 | P0 | R0 | M1 实现 bootstrap verifier | backend-dev | task-002 | ✅ 完成（r1 待复测） | `scripts/bootstrap-core-verifier.py`, `scripts/bootstrap_verifier*.py`, `tests/bootstrap_verifier/` | `evidence/m1/held-out-blocker-review.md` |
| task-004 | P0 | R0 | M1 负向 fixtures 与反作弊验证 | tester | task-003 | 🟢 就绪（复测） | negative fixtures, receipt | `evidence/m1/held-out-blocker-review.md` |
...
| 任务 | 次数 | 原因 | 审查人 |
|------|------|------|--------|
| task-003 | 1 | held-out 发现 command/trusted-files 未闭合、receipt/input 重叠和重复 check-id 三个 fail-closed 缺口；r1 已修复，等待独立复测 | Tester |
