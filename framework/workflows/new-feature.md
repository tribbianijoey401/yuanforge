...
→ 涉及高影响 UI 时执行 Repository Capability Audit 与内容/页面边界建模
→ UI Designer 通过 Prototype Convergence 冻结可定位的 Presentation Contract
→ Risk 命中时 Independent Review 同一 Contract
...
> **Writer 语义：** `frontend-dev` 与 `backend-dev` 至少启用一个作为唯一 Implementation Writer，由 Conductor 按 Feature 涉及的代码域选择。跨 Module 时才启用 `architect`；涉及 UI 时必须先启用 `ui-designer` 冻结 Presentation Contract（`presentation_contract: frozen`），之后才能 dispatch `frontend-dev`；其余 `optional_agents` 仅在 Risk Signal 命中时启用。
