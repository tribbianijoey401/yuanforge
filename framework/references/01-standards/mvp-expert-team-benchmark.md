...
| 记忆系统 | pitfalls.jsonl + 自学习闭环 | `docs/MEMORY.md` + `knowledge-injection` 已覆盖 | 基本等价 |
...
- **`policies/visual-absolutes.md`** — P0 视觉绝对禁令 VA-1~VA-5：
  ...
  - VA-5 禁止弹跳/弹性缓动
  - 绑定 G2/G3 门禁
- **`policies/verdict-protocol.md`** — RoleVerdict 结构化裁决协议：`verdict/pass|fail + blocking[] + advisory[] + evidence[]` + 诊断式打回 + 过度设计护栏 + Bounded ≤3 轮

### 改写 5 个合约 + AGENTS.md 的 Checklists（不碰 role-scorecard / anti-patterns.md）
- `agents/ui-designer.md`：新增「视觉绝对禁令（P0）」段，禁止事项补 VA-1~5，门禁加 emoji 正则扫描
- `agents/ux-reviewer.md`：新增「Emoji 正则扫描（VA-1）」+「五源对齐」，输出格式前置 verdict-protocol 结构化块，门禁加 VA-1 扫描 + 五源对齐
- `agents/frontend-dev.md`：禁止事项补 emoji/渐变/硬编码/弹跳，新增「自检循环 lint→type-check→test ≤3 轮」+ emoji 扫描，门禁加自检 + 扫描
- `agents/quality-auditor.md`：审计表加「代码组织」行（分层/≤300 行/单一职责/入口只装配），新增「代码组织门禁」段，门禁改为五段
- `agents/architect.md`：新增「Spec 即契约（增强要求）」段（Design Token 锁定 / EARS 验收 / 内嵌已知坑 / e2e 验证 / OPEN-DECISIONS 登记 + 3 slug）
