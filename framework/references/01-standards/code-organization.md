...
前端在 Vue / Next.js / Taro / Nuxt 等 Framework 中同样遵循职责分层：`pages` 只做页面组装，复用 UI 进入 `components`，API 调用统一进入 `services`，通用逻辑进入 `utils`，共享数据结构进入 `types`，跨页面 State 进入 `stores`。组件按 Feature 拆分，页面不堆积 Business Logic。
