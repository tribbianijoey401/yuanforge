...
> **vNext Scope：** `core.md` 是唯一 Always-on Policy。本文件保留 main 中成熟的工程纪律，作为按需 Policy Library；Routing 根据 Work 类型选择相关 Rule，禁止每轮全文注入。
...
- Security Auditor：按 R0/R1/R2 分级投入，不通过 → Blocker
...
- 三次审查未通过 → 在 STATUS.md 标记阻塞，Conductor 通知用户
...
- 不依赖"上一个 Agent 记得什么"
- Subagent 通过读取项目文档（STATUS.md、ARCHITECTURE.md）了解上下文
...
Dev 完成 Task ──→ │
                    ├── Security Auditor ──→ 🔴 Blocker (按 R0/R1/R2 分级执行)
                    │
...
| 检查 | 条件 | 动作 |
|------|------|------|
| 空闲检测 | 30 分钟无 WORK 状态变化 | 通知用户："暂停中，无状态变化" |
...
L0 项目循环: backlog → Feature → archive → backlog
 └─ L1 Feature 流水线循环（Conductor 元循环）: 读 WORK → 选 Task → Dispatch → 等结果 → 更新状态 → 回到读
...
| 状态 | 原因指针指向 | 证据文件 |
|------|-------------|---------|
| 🔄返工 | WORK::审查结果 段 | workspace/审查报告/T{id}-review-{role}-r{round}.md |
| ❌阻塞 | WORK::阻塞记录 段 | WORK.md 阻塞记录表 |
| 🔨超时回退 | events/*.jsonl | 最近 DISPATCH 事件 |
| 🟢就绪 | WORK::上下文传递 段 | 上游产出文件 |
