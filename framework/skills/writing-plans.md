...
- Requirement / Acceptance 需要自包含 Contract：读取 `references/01-standards/spec-as-contract.md` 的 Self-contained Spec 与 E2E Verification Section。
- 存在未决选择：读取 `references/01-standards/open-decisions-register.md` 的 Trigger、Field 与 Resolve Section。
- Plan 可能注入过多 Context：读取 `references/01-standards/context-engineering.md` 的 JIT 与 Compaction Section。
- 涉及 Module Boundary：读取 `references/01-standards/code-organization.md` 的 Layer 与 Dependency Section。

未命中 Signal 时不读取上述 Reference。

## Inputs

- `docs/WORK.md` 的 Goal、Scope、Acceptance、Risk 与 Assumption
...
7. Plan 写入 `docs/WORK.md`；只有 Complex Work 才增加 Optional Task Board。
