...
Conductor 收尾:
  ...
  2. 填 SESSION_LOG「任务完成情况」
  3. 有未完成 → PROGRESS 标记「未完成，续接至下一会话」
  ▼
  3. PROGRESS.md 移至历史会话（全部终态）/ 保持当前（有未完成任务）
  ▼
[如有未完成任务 → 下一会话]
  新 Conductor 读上一会话 SESSION_LOG → 继承未完成任务 → 初始化新 TASK_BOARD
```
...
SESSION_LOG.md                         Conductor 继承逻辑:
  T01 ✅测试通过       ────────→         1. 读上一个 Session 的 SESSION_LOG
  T02 ✅完成                               的「任务完成情况」表
...
| 角色 | 读 | 写 |
|------|-----|-----|
| Conductor | 全部 | 初始化任务行、promote 状态、写阻塞、注入诊断包 |
