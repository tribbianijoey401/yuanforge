...
| ID | 优 | 任务 | 角色 | 依赖 | ⏱超时(分) | 状态 | 产出 |
|----|----|------|------|------|-----------|------|------|
| T01 | P2 | [任务名] | [product-analyst/architect/ui-designer/frontend-dev/backend-dev/spec-reviewer/security-auditor/quality-auditor/ux-reviewer/tester/doc-engineer] | [-] | [默认见下表] | [状态] | [文件路径] |
...
| 4 | grep 上下文传递中写给自己 T-ID 的行（~3行） | ✅ | 知道上游传了什么 |
| 5 | 读铁律 + 角色合约（~2KB） | ✅ | 知道规则和禁止事项 |
| 6 | 读 pitfalls.md / knowledge/pitfalls/（~2KB） | ✅ | 避免重复踩坑 |
| 7 | 读上游产出物文件（按需） | — | 获取具体实现细节 |
...
| 角色 | 读 | 写 |
|------|-----|-----|
| Conductor | 全部 | 初始化任务行、promote 状态、写阻塞/审查结果/调度状态/派发日志、注入诊断包、更新当前状态快照、渐进更新 SESSION_LOG、写 Events |
