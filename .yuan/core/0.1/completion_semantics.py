...
def evidence_satisfies_ac(
    ...
    *,
    ...
    environment_fingerprint: str,
    observed_now: datetime,
    attempts_by_id: dict[str, dict[str, Any]] | None = None,
) -> bool:
    """Return true only for one fully bound, independently verified AC proof."""
    if not _valid_evidence_for_ac(
        work,
        ac,
        evidence,
        artifact_sha256,
    ...
    if not (
        ...
        and immutable_binding_matches(
            source.get("harness_binding"), work["harness_binding"]
        )
        and immutable_binding_matches(
    ...
    *,
    ...
    for ac in (item for item in work["acceptance_criteria"] if item["required"]):
        ...
        if not isinstance(active_fingerprint, str) or not any(
            evidence_satisfies_ac(
                work,
                ac,
                item,
                artifact_sha256=current_artifact_sha256,
                environment_id=environment_id,
                environment_fingerprint=active_fingerprint,
