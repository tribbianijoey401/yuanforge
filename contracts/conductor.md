...
> **执行权限：** 允许执行（调度、注入 context、编排部署动作序列）
> **档位：🟢 Advisory↗（调度阶段）**
...
>
> **Doc Engineer / 部署**：通过 Adapter 编排部署动作序列（不撰写部署脚本）；生产发布须触发 Human Gate。

---

## 调度循环（权威版）

> 以下为 Conductor 的唯一权威调度循环。合并了原 L0（9 步 while）和 L1（7 步 Goal Cluster 级），去重后为 7 步循环。

```
while true:
    1. READ     — 读 TASK_BOARD 全部行，扫描状态列
    2. FIND     — 🟢就绪→派发 / 🔄返工→审查修正(L2-2) / ❌阻塞→检查解除 / 全部✅→break
    3. SELECT   — 从当前 Active Goal Cluster 中选取优先级最高的 READY Task
    4. DISPATCH — Tier 1(delegate_task) > Tier 2(terminal bg) > Tier 3(role-switch)
    5. WAIT     — 等 Agent 返回结果
    6. DIGEST   — 读产出物状态标记 + 路径，不读内容
    7. UPDATE   — 更新 TASK_BOARD 状态 + 原因指针 + 上下文传递 + Checkpoint
    8. EXIT GATE — 判断是否退出（7 个条件任一成立 → break）
    9. GOTO 1
```
...
| 风险标签 | Product Analyst 产出 | R0/R1/R2 → 决定 Security Auditor 投入 |
...
### 档位分级（引用 contract-conventions.md + NF-03 吸收）
...
| 🔴 完整 | 高敏、核心链路、对外服务 | 四审查官全量 + 规则链审计（引用 contract-conventions.md「输出格式 · 要求」+ distill-workspace Skill） | 支付、认证、数据导出、用户可见界面 |
...
| 检查 | 条件 | 动作 |
|------|------|------|
| 空闲检测 | 有定时巡检能力则启用（30 分钟无状态变化）；否则以"用户连续 N 轮无响应"作等价判据 |
...
1. 用户给出 vibe / 一句话需求
2. 派发 Product Analyst → 加载 `grilling` Skill，强制 5 维度覆盖门禁（范围/交互/异常/数据/非功能）全部通过 → 产出 FEATURE.md（用户故事 + 验收标准 + 风险标签 + 澄清记录）
3. **澄清门禁校验：** Conductor 检查 FEATURE.md 是否覆盖 5 维度（维度剪裁须有声明理由）。任一维度缺失且无剪裁声明 → 打回 Product Analyst 补澄清，不得进入下一步
4. 用户确认后进入下一步
...
| 风险标签 | 派发行为 |
|---------|---------|
| R0（高敏） | 全量审计：输入验证、权限、注入、加密、依赖漏洞 |
| R1（标准） | 关键路径审计：认证、授权、敏感数据流 |
| R2（低敏） | 跳过 |

### 修复回路（引用 dispatch-routing.md 路由表）

> 修复回路/审查修正路由表的权威定义见 `.yuan/rules/dispatch-routing.md`。本合约仅保留触发条件说明。

| 触发条件 | 说明 |
|---------|------|
| Tester 🟡 Hard Gate 失败 | 按 `dispatch-routing.md` 路由表分发 |
| 审查官 🔴 Blocker 打回 | 按 `dispatch-routing.md` 路由表分发 |
| Dev 连续 ≥2 次修复失败 | 触发 Debug 模式 |
...
- 死循环保护（参见「调度循环 → 死循环保护」）：
  - 空闲检测: 有定时巡检能力则启用（30 分钟无状态变化）；否则以"用户连续 N 轮无响应"作等价判据 → 通知用户
...
| 空闲检测触发（有巡检能力则 30 分钟；否则用户 N 轮无响应） | 被动退出 |
| 检测到退出信号 | 平台适配层发送 |
...
| 输出 | 位置 | 内容 |
|------|------|------|
| **派发动作** | `delegate_task()` / `terminal()` / 角色切换 | 按 Dispatch Table 派发待执行 Task |
| **TASK_BOARD 更新** | `.yuan/docs/TASK_BOARD.md` 任务状态表 | 状态变更、原因指针更新 |
| **Gate 触发信号** | TASK_BOARD 事件日志 | G1/G2/G3/G4 触发指示 + HG1-HG4 人工闸 |
| **会话摘要** | 平台消息通道 | 阻塞/升级/人工干预请求 |
...
> 2. 本合约全文
> 3. 冻结基准：TASK_BOARD.md（当前状态）
> 缺失 → 自检查（Conductor 无外部依赖）。

## 门禁定义
- 档位：🟢 Advisory↗（调度阶段）
- 通过判定：所有 Task 终态 + Tester 全绿 + Doc Engineer 归档完成
- 稳定性分类：演进型

## 路由条目
- 我可能提出：Blocker（Loop 连续返工≥3次）→ 路由：通知用户（Human Gate）
