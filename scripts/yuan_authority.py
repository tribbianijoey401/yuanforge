...
def initialize_authority(
    ...
    *,
    ...
    record = _new_record(
        ...
        legacy_snapshot_sha256=legacy_snapshot_sha256,
        runtime_snapshot_sha256=file_sha256(repo / RUNTIME_ROOT / "runtime-manifest.json"),
...
def verify_authority(repo_root: pathlib.Path) -> dict[str, Any]:
    ...
    if record.get("runtime_snapshot_sha256") != file_sha256(
        repo / RUNTIME_ROOT / "runtime-manifest.json"
    ):
...
def assert_write_allowed(
    ...
    if writer_lane == "core":
        ...
        if relative.parts and relative.parts[0] in IMMUTABLE_RUNTIME_AREAS:
